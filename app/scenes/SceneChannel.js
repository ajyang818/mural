...
SceneSceneChannel.artInfoTimer = null;
...
function extractQualities(input) {
  ...
  var arts = extractStreamDeclarations(input);
  for (var i = 0; i < arts.length; i++) {
    result.push({
      'id': extractQualityFromStream(arts[i]),
      'url': extractUrlFromStream(arts[i])
    });
  }
...
SceneSceneChannel.prototype.handleHide = function() {
  ...
  window.clearInterval(SceneSceneChannel.artInfoTimer);
};
...
SceneSceneChannel.prototype.handleFocus = function() {
  ...
  $('#art_info_name').text(SceneSceneBrowser.selectedChannel);
  $("#art_info_title").text("");
  $("#art_info_viewer").text("");
  $("#art_info_icon").attr("src", "");
  SceneSceneChannel.updateStreamInfo();

  SceneSceneChannel.artInfoTimer = window.setInterval(SceneSceneChannel.updateStreamInfo, 10000);
...
SceneSceneChannel.prototype.handleKeyDown = function(keyCode) {
  ...
  if (SceneSceneChannel.state != SceneSceneChannel.STATE_PLAYING) {
    ...
    switch (keyCode) {
      ...
      case sf.key.CH_DOWN:
      case sf.key.N4:
        if (SceneSceneChannel.isPanelShown() && SceneSceneChannel.qualityIndex < SceneSceneChannel.getQualitiesCount() - 1) {
          SceneSceneChannel.qualityIndex++;
          SceneSceneChannel.qualityDisplay();
        }
        break;
...
SceneSceneChannel.onRenderError = function(RenderErrorType) {
  if (SceneSceneChannel.quality == "High" || SceneSceneChannel.quality == "Medium" || SceneSceneChannel.quality == "Low") {
...
SceneSceneChannel.updateStreamInfo = function() {
    ...
    if (xmlHttp.readyState === 4) {
      if (xmlHttp.status === 200) {
        try {
          ...
          $("#art_info_title").text(response.art.channel.status);
          $("#art_info_viewer").text(addCommas(response.art.viewers) + ' ' + STR_VIEWER);
          $("#art_info_icon").attr("src", response.art.channel.logo);
        } catch (err) {
  ...
  xmlHttp.open("GET", 'https://api.twitch.tv/kraken/arts/' + SceneSceneBrowser.selectedChannel, true);
